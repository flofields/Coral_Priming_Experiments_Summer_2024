...
## Import data
```{r, warning = FALSE}
path.p <- "data/Physiology/PR/20240716_17" #the location of all your respirometry files 
...
r_plot<-Resp %>%
    ggplot(., aes(x = Timepoint, y = -micromol.cm2.h, colour=Treatment)) +
    geom_point(aes(fill=Genotype, group=Genotype), pch = 21, size=2, alpha=0.3) + 
    xlab("Timepoint") +
    facet_wrap(vars(Species), scales = "free")+
    #scale_fill_manual(name="mtORF", values=c("orange","black","cyan", "blue", "green"))+
    #scale_color_manual(name="mtORF", values=c("orange","black","cyan", "blue", "green"))+
...
p_plot<-Photo %>%
    ggplot(., aes(x = Temp.Cat, y = micromol.cm2.h, colour=mtORF)) +
    geom_point(aes(fill=mtORF, group=fragment_ID), pch = 21, size=2, alpha=0.3) + 
    xlab("Temperature") + 
    scale_fill_manual(name="mtORF", values=c("orange","black","cyan", "blue", "green"))+
    scale_color_manual(name="mtORF", values=c("orange","black","cyan", "blue", "green"))+
...
## Import data
```{r, warning = FALSE}
path.p <- "data/PR/Dec_TPC" #the location of all your respirometry files 
...
# Load PI curve sample metadata (i.e., which corals were in which runs)
sample.info <- read_csv(file = "data/PR/Dec_TPC/Dec_PR_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "data/PR/Dec_TPC/Dec_PR_run_metadata.csv")
...
# Import surface area data
sa <- read_csv(file = "data/Dec_TPC_surface.area.calc.csv")
...
Photo <- left_join(Photo, morph, by="Number")
Resp <- left_join(Resp, morph, by="Number")

Photo <- na.omit(Photo) %>% select("fragment_ID...1", micromol.cm2.h, mtORF, Temp.Cat)
colnames(Photo)[1] <- "fragment_ID"
Photo$Timepoint <- "Dec"
Photo$measure <- "Photo"
Resp <- na.omit(Resp) %>% select("fragment_ID...1", micromol.cm2.h, mtORF, Temp.Cat)
colnames(Resp)[1] <- "fragment_ID"
Resp$Timepoint <- "Dec"
Resp$measure <- "Resp"
