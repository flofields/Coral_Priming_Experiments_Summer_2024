...
homog.vol <- read_csv("C:/Users/flo_f/OneDrive - University of Rhode Island/GitHub/Coral_Priming_Experiments_Summer_2024/data/physiology/Airbrushing_data.csv")%>%
  select(Species, Sample.ID, Homogenate.Vol)
...
rename(Sample.ID = Colony_id)
chl1 <- full_join(chl, sa)

colnames(chl1)[2] <- "Fragment_ID"
...
colnames(df_summary2_avg)[3] <- "Priming_Treatment"


ggplot(df_summary2_avg, aes(x = Priming_Treatment, y = chla_mean, 
  ...
  theme(axis.text.x = element_text(angle = 35, hjust = 1), 
      panel.spacing = unit(0.3, "lines"), 
      panel.border = element_rect(color = "black", fill = NA)) +
labs(x = "Priming Treatment", y = expression("Average Chlorophyll a (µg/cm"^2*)"))
