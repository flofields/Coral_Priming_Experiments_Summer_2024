...
## Import data
```{r, warning = FALSE}
path.p <- "../data/Physiology/PR_use/20240708_09" #the location of all your respirometry files 
...
sample.info <- read_csv(file = "../data/Physiology/PR_use/pr_sample_metadata.csv")

# Load PI curve run metadata (i.e., light levels and interval times for each run)
run.info <- read_csv(file = "../data/Physiology/PR_use/pr_run_metadata.csv")
...
# Rename sample id to fragment_ID
colnames(metadata)[1] <- "fragment_ID"

# Select only certain columnns
metadata <- metadata %>%
  select(fragment_ID, Run, Treatment, Species, Chamber.Vol.L, Date, Start.time, Stop.time, Light_Percent, Temp.Cat)
...
thin_par <- 20
thin_par <- max(1, abs(thin_par))  # This ensures thin_par is at least 1

# Thin data for all samples
df <- df %>%
  mutate(thin_data = map(data, ~ {
    if (nrow(.) > 0) {
      slice(., seq(1, nrow(.), by = thin_par))
    } else {
      .  # Return the original data if it's empty
    }
  }))
...
pr <- left_join(
  select(df.out, fragment_ID, Light_Percent, micromol.L.s),
